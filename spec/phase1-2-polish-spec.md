...
### 4.6 reactive 模式等待过程不透明

当前 reactive 模式在页面上基本只有“正在分析中”，直到最终回答开始流式输出。短问题体验可以接受，但如果 reactive 内部进行了多轮工具调用，用户可能会面对较长时间空白等待。

优化目标：

- 不把 reactive 做成完整 deliberative，不展示任务规划。
- 为 reactive 增加“轻量透明流程”，让用户知道当前在理解问题、调用工具、整理回答。
- 尽量复用现有前端展示，不大改当前已经满意的页面结构。
- 如果 reactive 没有工具调用，只展示简短状态；如果有工具调用，则展示轻量版 Act / Observe。
...
#### 5.1.2 reactive 轻量透明流程

reactive 不引入 `plan_node`，也不展示任务规划面板。它保留“快速响应”的定位，但补充轻量状态反馈，避免用户长时间只看到“正在分析中”。

实现约定：

- **不新增 `reactive_status` SSE 事件类型**，避免前端改动过大。
- reactive 的“正在理解问题 / 正在整理回答”等轻量状态复用现有 `think` 事件。
- reactive 的工具调用继续复用现有 `act` / `observe` 事件。
- 前端根据 `processing_mode == "reactive"` 将这些事件展示为“快速响应过程”，不显示 PlanPanel。
- 为降低实现复杂度，reactive 一进入就推送一条轻量 `think` 状态，不做“超过 3 秒才展示”的计时逻辑。短任务中该状态会很快被最终回答覆盖；长任务中用户能立即感知系统正在处理。
...
assess：识别为 reactive
  ↓
think：正在理解问题并判断是否需要工具
  ↓
reactive_agent：调用带工具的 LLM
  ├── 无工具调用：think：正在整理回答 -> extract_response
  └── 有工具调用：act：调用工具 -> observe：工具结果摘要 -> reactive_agent 继续判断
  ↓
extract_response：提取最终回答
  ↓
synthesize：流式展示最终回答
  ↓
done
...
| 时机 | 事件 | 内容 |
|---|---|---|
| assess 后进入 reactive | `think` | `正在理解问题并判断是否需要工具` |
| reactive_agent 调用 LLM 前 | `think` | `正在生成快速回答` |
| 检测到 tool_calls | `act` | 工具名 + 参数摘要 |
| tools 执行完成 | `observe` | 工具结果摘要 |
| 达到工具上限或无工具调用 | `think` | `正在整理最终回答` |
| extract_response 完成 | `synthesize` | 最终回答 |
| 任意结束路径 | `done` | 结束状态与耗时 |
...
```text
[快速响应] 正在理解问题...
[工具调用] rag_search：检索相关资料
[工具结果] 找到 3 条相关内容，正在整理回答...
[回答] ...
```
...
- 不展示 PlanPanel。
- 不展示完整复杂推理链。
- 只展示用户关心的执行状态：理解、工具、结果、回答。
- 若发生工具调用，复用现有 `act` / `observe` 展示，不新增复杂 UI。

这样既参考成熟 Agent 的“状态可见”设计，又不会把 reactive 做得过重。

---

#### 5.1.3 成熟 Agent 展示形态参考

成熟 Agent 通常会区分“轻任务”和“复杂任务”的可视化层级：
...
| 模式 | 展示粒度 | 是否展示 Plan | 适用问题 |
|---|---|---|---|
| reactive | 轻量状态 + 工具摘要 | 否 | 简单问答、事实查询、少量工具调用 |
| deliberative | 完整 Plan / Think / Act / Observe | 是 | 深度分析、财务研究、多步骤报告 |
...
说明：本节主要针对 deliberative 路径的 `think_node`。reactive 路径不经过 `think_node`，但需要在 `reactive_agent` 前后补充轻量 `think` / `act` / `observe` 事件，用于前端展示“快速响应过程”。
...
reactive 路径可以执行 LLM 返回的多个 tool calls，但需要继续受 `max_reactive_tool_calls` 限制。前端展示时只显示工具名、参数摘要和结果摘要，避免把快速响应过程做得过重。
...
#### 5.7.5 Embedding 模型策略

长期记忆检索使用 DashScope 的 `text-embedding-v4` 模型。

配置约定：

| 配置项 | 说明 |
|---|---|
| `EMBEDDING_PROVIDER` | 固定为 `dashscope` |
| `EMBEDDING_MODEL` | `text-embedding-v4` |
| `EMBEDDING_DIMENSION` | 按实际返回向量维度配置，默认使用项目现有配置 |
| `DASHSCOPE_API_KEY` | 从环境变量读取 |

实现要求：

- `LongTermMemory._embed()` 在 `embedding_provider == "dashscope"` 时调用 DashScope embedding API。
- API Key 从环境变量读取，不写入代码或配置文件。
- 如果环境变量缺失或 API 调用失败，长期记忆检索/保存应降级失败，不影响 Agent 主流程。
- 测试中允许使用 `embedding_provider="mock"`，但实际演示默认使用 DashScope `text-embedding-v4`。

本项目的长期记忆定位是“简历项目亮点”：重点展示经验保存、语义检索和方法经验注入闭环，不追求生产级记忆系统。
...
- `assess`
- `plan`，仅 deliberative
- `think`，deliberative 必有；reactive 可选用于轻量状态
- `act`，发生工具调用时推送，reactive / deliberative 均可使用
- `observe`，工具执行后推送，reactive 中展示为工具结果摘要
- `memory`，仅注入长期经验时
- `synthesize`
- `error`，发生异常时
- `done`
...
#### 5.9.2 reactive 前端轻量展示

当前前端整体展示已经比较满意，本次不建议大改已有页面结构。reactive 体验优化应优先通过“复用现有组件 + 增加轻量事件内容”完成。
...
- 不新增 PlanPanel。
- 不改变 deliberative 当前展示。
- reactive 可复用现有 `ThinkPanel` 或 `AgentMessage` 中的过程区块。
- 对 reactive 过程使用更轻量的标题，例如：`快速响应过程`。
- 默认折叠工具详情，只显示工具名和结果摘要。
- 如果 reactive 没有工具调用，只显示一条“正在整理回答”即可。
...
如果不想新增视觉模块，也可以把这些状态作为已有思考面板中的简短条目展示：
...
- 不生成 plan
- 快速回答
- 如果回答耗时较短，可以只显示简短 loading
- 一进入 reactive 即推送轻量状态，例如“正在理解问题 / 正在整理回答”
- 发送 `done`
...
### 第一批：稳定 ReAct 主流程与 reactive 体验

1. 为 reactive 路径增加轻量 `think` / `act` / `observe` 事件推送，展示“快速响应过程”。
2. 优化 `synthesize_node`，确保 final_answer 不为空，并设置 `is_finished=True`。
3. 优化 `route_after_observe`，增加清晰收敛条件。
4. 优化 `observe_node`，避免无效工具结果推进步骤。
5. 增加重复工具调用抑制。

### 第二批：工具稳定性

6. 优化 `browser_use` 的配置、错误提示和降级标识。
7. 优化 `act_node` 工具异常处理。
8. 标准化工具结果返回格式。

### 第三批：记忆系统打磨

9. 实现 DashScope `text-embedding-v4` embedding 调用，API Key 从环境变量读取。
10. 优化长期记忆保存条件。
11. 增加相同 query 去重。
12. 优化记忆注入提示词，强调“方法经验”。

### 第四批：SSE 和前端体验

13. 保证异常路径也发送 `error` 和 `done`。
14. 前端复用现有 `ThinkPanel` 或 `AgentMessage` 展示“快速响应过程”，不新增 PlanPanel。
15. 前端兼容工具失败、memory 事件、无 plan 直接回答等情况。
16. 优化透明面板默认折叠和错误展开。
...
| 记忆亮点 | 相似任务能注入历史方法经验 |
| 前端体验 | 透明过程可见，loading 不假死；reactive 任务开始后应尽快收到轻量状态事件或最终回答；任意结束路径必须收到 `done` |
...
```text
基于 LangGraph 构建企业知识库分析 Agent，实现 reactive/deliberative 双路径任务处理；针对简单查询设计轻量透明响应流程，针对复杂分析任务展示完整 Plan、Think、Act、Observe 推理链；集成 RAG 检索、Python 计算、网页搜索、浏览器访问和文件操作等工具；基于 DashScope text-embedding-v4 实现长期经验记忆，在相似分析任务中自动注入历史方法经验；通过工具失败降级、重复调用抑制和部分报告生成提升复杂任务演示稳定性。
```
