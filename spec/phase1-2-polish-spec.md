...
assess：识别为 reactive
  ↓
reactive_status：正在理解问题
  ↓
reactive_agent：判断是否需要工具
  ├── 无工具调用：reactive_status：正在整理回答 -> synthesize
...
- 若 reactive 在 2-3 秒内返回，可只显示简短 loading；若超过 3 秒，显示轻量流程。
...
- 如果耗时超过 3 秒，显示 reactive 轻量状态，例如“正在理解问题 / 正在整理回答”
- 发送 `done`
