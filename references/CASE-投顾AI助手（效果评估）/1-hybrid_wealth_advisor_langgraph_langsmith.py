...
# 遗留兼容性补丁：修复 dashscope 响应对象在构造 HTTPError 时触发 KeyError('request') 的问题
# 当前脚本已改用 DeepSeek 平台，但如果后续切回 DashScope/Tongyi，保留此补丁可避免真实错误被掩盖
try:
    from dashscope.api_entities.dashscope_response import DashScopeAPIResponse
    _original_getattr = DashScopeAPIResponse.__getattr__
    ...
    DashScopeAPIResponse.__getattr__ = _patched_getattr
except Exception:
    pass
...
if __name__ == "__main__":
    ...
    print("使用模型：deepseek-v4-flash\n")
    
    # 显示 LangSmith 状态
    if LANGSMITH_ENABLED:
        print("[已启用] LangSmith 追踪已启用")
        print(f"  项目: {LANGSMITH_PROJECT}")
        print(f"  查看追踪: https://smith.langchain.com\n")
    else:
        print("[未启用] LangSmith 追踪未启用")
        print("  要启用追踪，请设置环境变量:")
