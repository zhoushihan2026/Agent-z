...
# 使用脚本所在目录作为基准，避免因工作目录不同导致找不到主模块
_hybrid_module_path = os.path.join(
...
if __name__ == "__main__":
    ...
    try:
        ...
        if created_dataset:
            print(f"[成功] 测试数据集准备完成: {dataset_name}")
        else:
            print(f"[错误] 数据集准备失败")
            print()
            print("请检查:")
    ...
    try:
        existing_examples = list(client.list_examples(dataset_name=dataset_name))
        if len(existing_examples) == 0:
            print(f"[错误] 数据集为空，无法运行评估")
            print("请确保数据集包含测试用例")
            exit(1)
    except Exception as e:
        print(f"[错误] 无法验证数据集: {str(e)}")
        print("请确保数据集存在且可访问")
        exit(1)
