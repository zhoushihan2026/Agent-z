        ...
        async with self.lock:
            try:
                ...
                # 元素交互操作
                    ...
                    # 调试：点击后等待页面响应，保存截图和 HTML
                    import os, time as _time
                    page = await context.get_current_page()
                    await page.wait_for_load_state(timeout=5000)
                    _time.sleep(1)  # 额外等待日历控件弹出

                    debug_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), "debug_screenshots")
                    os.makedirs(debug_dir, exist_ok=True)
                    ts = _time.strftime("%Y%m%d_%H%M%S")

                    # 保存截图
                    screenshot_path = os.path.join(debug_dir, f"click_{index}_{ts}.png")
                    await page.screenshot(path=screenshot_path, full_page=True)
                    logger.info(f"📸 调试截图已保存: {screenshot_path}")

                    # 保存 HTML
                    html_path = os.path.join(debug_dir, f"click_{index}_{ts}.html")
                    ...
                    # 额外调试：检查是否有日历相关元素
                    calendar_check = await page.evaluate("""
                        () => {
                            const results = [];
                            // 查找日历相关 class/id
                            const selectors = [
                                ...
                                '[class*="DateInput"]', '[class*="DateRangePicker"]',
                                '.rdrCalendarWrapper', '.rdrDateRangeWrapper',
                                '.ant-calendar', '.ant-picker-calendar',
                                '[class*="el-date"]',
                            ];
                    ...
                    """)
                    if calendar_check:
                        logger.info(f"🔎 发现日历相关元素: {json.dumps(calendar_check, ensure_ascii=False, indent=2)}")
                    else:
                        logger.warning("⚠️ 未发现任何日历相关元素")
