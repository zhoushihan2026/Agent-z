// Agent-z Sidebar 组件
// 基于 RAG-z 的 Sidebar 改造，适配 Agent-z 的会话管理
  ...
  return (
    <div
      ...
      className={`group relative flex items-center gap-2.5 px-3 py-2.5 mx-2 rounded-lg cursor-pointer transition-all duration-150 ${
        isActive ? "bg-[#EEF2FF]" : "hover:bg-[#F0F1F3]"
      }`}
    >
      {isActive && (
      ...
      <div className="opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        <SessionMenu
          onRename={() => { setEditTitle(session.title); setIsEditing(true); }}
          onDelete={onDelete}
        />
      </div>
    </div>
...
export function Sidebar({
  ...
  return (
    <aside
      className="h-full bg-[#FAFBFC] border-r border-[#F0F1F3] flex flex-col shrink-0 overflow-hidden"
      style={{
